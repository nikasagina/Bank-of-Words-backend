...
public class StatisticsService {
    ...
    @Autowired
    public StatisticsService(WordDAO wordDAO, UserDAO userDAO, StatisticsDAO statisticsDAO,
    ...
    public ResponseEntity<?> getUserStatistics(String authHeader) {
    ...
    public ResponseEntity<?> getGlobalStatistics(String authHeader) {
        String token = authHeader.replace("Bearer ", "");
        String username = jwtUtil.getUsernameFromToken(token);
        if (jwtUtil.validateToken(token, username)) {
            Map<String, Object> response = new HashMap<>();
    ...
    public ResponseEntity<?> getUserActivity(String authHeader) {
        String token = authHeader.replace("Bearer ", "");
        String username = jwtUtil.getUsernameFromToken(token);
        if (jwtUtil.validateToken(token, username)) {
            Map<String, Object> response = new HashMap<>();
